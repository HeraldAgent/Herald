...
import { deduplicateNews, filterRelevantNews, rankNewsByImpact } from "./src/signals/filter.js";
...
async function scan() {
  ...
  logger.info(`Fetched ${all.length} items → ${relevant.length} relevant → top ${Math.min(ranked.length, 10)} queued for analysis`);

  const topItems = ranked.slice(0, 10);
  if (topItems.length === 0) {
    logger.info("No relevant news this cycle");
    return;
  }
